...
function __construct(){
	...
	public function index()
	{
		...
		$data['mpartner']=$this->M_partner->selectAll();
		$this->load->view('partner/v_partner',$data);
	}
	public function upload_process(){
		...
		if ( ! $this->upload->do_upload('image')){
			...
			$this->db->insert('mpartner', $data_insert);
	...
	function editu() {
		...
		$e = $this->db->where('id', $id)->get('mpartner')->row();
		$kirim['id'] = $e->id;
		$kirim['judul'] = $e->judul;
	...
	function update(){
		...
		//if upload success
				...
				@unlink('./uploads/partner/'.$filelama);
				...
				$this->M_partner->update_data($where,$data,'mpartner');

				redirect('partner');
			}
			if (!empty($filelamam)) {
					@unlink('./uploads/partner/'.$filelamam);
					$id = $this->input->post('editid');
	...
	function hapus($id){
		$this->db->where('id',$id);
		$query = $this->db->get('mpartner');
		$row = $query->row();
		unlink("./uploads/partner/$row->image");
		$this->db->delete('mpartner', array('id' => $id));
		$this->session->set_flashdata('message', '<div id="success" class="alrt-success alert-dismissible col-md-3" data-dismiss="alert" aria-hidden="true">
			<i class="icon fa fa-check"></i>
