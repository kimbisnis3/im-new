...
function __construct(){
	...
	public function index()
	{
		...
		$data['mservices']=$this->M_services->selectAll();
	...
	public function upload_process(){
		...
		if ( ! $this->upload->do_upload('image')){
			...
			$this->db->insert('mservices', $data_insert);
	...
	function editu() {
		...
		$e = $this->db->where('id', $id)->get('mservices')->row();
		$kirim['id'] = $e->id;
		$kirim['judul'] = $e->judul;
	...
	function update(){
		...
		//if upload success
				...
				@unlink('./uploads/services/'.$filelama);
				...
				$this->M_services->update_data($where,$data,'mservices');

				redirect('services');
			}
			if (!empty($filelamam)) {
					@unlink('./uploads/services/'.$filelamam);
					$id = $this->input->post('editid');
	...
	function hapus($id){
		$this->db->where('id',$id);
		$query = $this->db->get('mservices');
		$row = $query->row();
		unlink("./uploads/services/$row->image");
		$this->db->delete('mservices', array('id' => $id));
		$this->session->set_flashdata('message', '<div id="success" class="alrt-success alert-dismissible col-md-3" data-dismiss="alert" aria-hidden="true">
			<i class="icon fa fa-check"></i>
